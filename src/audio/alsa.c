...
static int alsa_renderer_init(int audioConfiguration, POPUS_MULTISTREAM_CONFIGURATION opusConfig, void* context, int arFlags) {
  ...
  snd_pcm_uframes_t buffer_size = 8 * period_size; // 160 ms buffer (increased from 60ms)
  ...
  CHECK_RETURN(snd_pcm_sw_params_set_start_threshold(handle, sw_params, buffer_size / 2)); // Start when buffer is half full
